        ...
        <div class="box box-solid">
          ...
          <div class="box-body no-padding">
            <ul class="nav nav-pills nav-stacked">
              <li><a href="<?php echo base_url(); ?>index.php/home/notifications"><i class="fa fa-inbox"></i> Inbox
                  <span class="label label-primary pull-right"><?php echo $data['notificationCount']; ?></span></a></li>
              <li class="active"><a href="<?php echo base_url(); ?>index.php/home/sentNotifications"><i
                      class="fa fa-envelope-o"></i> Sent
                  <span class="label label-info pull-right"><?php echo $data['sentNotificationCount']; ?></span></a>
              </li>
              <li><a href="<?php echo base_url(); ?>index.php/home/trashNotifications"><i class="fa fa-trash-o"></i>
                  Trash
        ...
        <div class="box box-primary">
          ...
          <!-- <div class="box-body table-responsive" style="margin-top: -54px;">
                        <table  id="example22" class="table table-hover table-striped" style="margin-top: 12px !important;">
                            ...
                            <tbody>
                                <form id="deleteMail" action="<?php echo base_url(); ?>index.php/admin_controller/deleteMail" method="POST">
                                    <?php foreach ($sentnotificationinfo as $notification) {
              $this->db->select('*');
              $this->db->from('users');
              $this->db->where('user_id', $notification->touser);
              $userinfo = $this->db->get()->row();
              ?>
                                    <tr>
                                        <td><input type="checkbox" name="mail[]" value="<?php if (!empty($notification->mail_id)) echo $notification->mail_id; ?>"></td>
                                        <td class="mailbox-star"><a href="#"><i class="fa fa-star-o text-yellow"></i></a></td>
                                        <td class="mailbox-name"><a href="<?php echo base_url(); ?>index.php/admin_controller/readmail?mail_id=<?php echo $notification->mail_id; ?>"><?php if (!empty($userinfo->fname)) echo $userinfo->fname; ?> <?php if (!empty($userinfo->lname)) echo $userinfo->lname; ?></a></td>
                                        <td class="mailbox-subject"><b><?php if (!empty($notification->subject)) echo $notification->subject; ?></b> - <?php if (!empty($notification->message)) echo $notification->message; ?>...</td>
                                        <?php if (isset($notification->forid) and $notification->forid != NULL) { ?>
                                        <td class="mailbox-name"><a href="<?php echo base_url(); ?>index.php/admin_controller/editwalktype?id=<?php echo $notification->forid; ?>"><i class="fa fa-link" aria-hidden="true"></i>Link</a></td>
                                        <?php } else { ?>
